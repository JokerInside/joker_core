...
function ESX.ShowNotification(message, notifyType, length)
    if GetResourceState("Joker-Notify") ~= "missing" then
        return exports["Joker-Notify"]:Jokers(notifyType, length, message)
    end

    print("[^1ERROR^7] ^5Joker Notify^7 is Missing!")
end

function ESX.TextUI(message, notifyType)
    if GetResourceState("joker_ui") ~= "missing" then
        return exports["joker_ui"]:TextUI(message, notifyType)
    end
...
function ESX.HideUI()
    if GetResourceState("joker_ui") ~= "missing" then
        return exports["joker_ui"]:HideUI()
    end
...
--		fuelLevel = ESX.Math.Round(exports['joker-fuel']:GetFuel(vehicle), 1),
