    ...
    local embedData = {
        {
            ...
            ["footer"] = {
                ["text"] = "| JokerInside | " .. os.date(),
                ["icon_url"] = "https://i.ibb.co.com/bLsBpMR/esxLogo.png",
            },
            ["description"] = message,
            ["author"] = {
                ["name"] = "JokerInside",
...
function ESX.UseItem(source, item, ...)
    if ESX.Items[item] then
        ...
        if itemCallback then
            ...
            if not success then
                return result and print(result) or print(('[^3WARNING^7] An error occured when using item ^5"%s"^7! This was not caused by Joker.'):format(item))
            end
        end
    else
